...
const ResolveHR = ({ employeeRepository, repository, setRepository, user, triggerNotification }) => {
  ...
  // Filter queries to only show HR-related ones, then separate into pending and non-pending
  const hrQueries = repository.filter((q) => q.queryType === QueryType.HRQUERY);
  const pendingQueries = hrQueries.filter((q) => q.queryStatus === QueryStatus.PENDING);
  const otherQueries = hrQueries.filter((q) => q.queryStatus !== QueryStatus.PENDING);
  ...
  return (
    ...
    <div className="ResolveHRContainer">
      <header>
        <h1>{questionlogo} HR Queries</h1>
        <p> Resolve HR-related questions and requests from employees</p>
      </header>
